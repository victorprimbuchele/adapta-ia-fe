...
function profileBreakdown(classItem: ClassSummary): Array<{ id: string; name: string; count: number }> {
...
function ClassCard({ classItem, onDelete }: { classItem: ClassSummary; onDelete: (classItem: ClassSummary) => void }) {
  const navigate = useNavigate();
  const profiles = profileBreakdown(classItem);
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="font-bold text-ink text-base font-heading">{classItem.name}</h3>
            <div className="flex items-center gap-2.5 mt-3 flex-wrap">
              <span className="text-xs text-muted">
                <span className="font-bold text-ink">{classItem.students.length}</span>{" "}
                aluno{classItem.students.length !== 1 ? "s" : ""}
              </span>
        ...
        <div className="flex items-center gap-2 flex-wrap flex-shrink-0">
          ...
          <button
            type="button"
            onClick={() => navigate(`/turmas/${classItem.id}/alunos/novo`)}
  ...
  return (
    <AppLayout>
        ...
        <div className="flex items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-muted text-sm mt-1">
              {isPending ? "Carregando..." : `${classes?.length ?? 0} turma${classes?.length === 1 ? "" : "s"} cadastrada${classes?.length === 1 ? "" : "s"}`}
            </p>
          </div>
          <button
            type="button"
        ...
        {isPending ? (
          ...
          <div className="space-y-4">
            {classes.map((classItem) => (
              <ClassCard key={classItem.id} classItem={classItem} onDelete={setClassToDelete} />
            ))}
          </div>
        )}
      </div>
      ...
      <ConfirmDialog
        ...
        title="Excluir turma?"
        description={`"${classToDelete?.name ?? "Esta turma"}" será excluída. Essa ação não pode ser desfeita.`}
