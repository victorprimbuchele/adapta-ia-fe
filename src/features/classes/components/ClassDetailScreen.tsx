...
export function ClassDetailScreen() {
  ...
  const { classDetail, students, isPending, isError } = useClassDetail(id);
  ...
  const school = schools?.find((s) => s.id === classDetail?.schoolId);
  const grade = grades?.find((g) => g.id === classDetail?.gradeId);
  ...
  const handleConfirmDeleteClass = () => {
    ...
    deleteClass.mutate(classDetail.id, {
  ...
  return (
    <AppLayout>
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-8 gap-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-ink font-heading">{classDetail.name}</h1>
                  <p className="text-muted text-sm mt-0.5">
                    {[grade?.name, school?.name].filter(Boolean).join(" · ")}
                  </p>
              ...
              <div className="flex items-center gap-2 flex-wrap flex-shrink-0">
                <button
                  type="button"
                  onClick={() => navigate(`/turmas/${classDetail.id}/editar`)}
                  title="Editar turma"
                ...
                <button
                  type="button"
                  onClick={() => navigate(`/turmas/${classDetail.id}/alunos/novo`)}
                ...
                {students.map((student, index) => (
                  <div
                    ...
                    className={`flex items-center gap-4 px-5 py-4 ${
                      index < students.length - 1 ? "border-b border-border-soft" : ""
                    }`}
                  >
                    ...
                    <div className="flex items-center">
                      <button
                        type="button"
                        onClick={() => navigate(`/turmas/${classDetail.id}/alunos/${student.id}/editar`)}
                        title="Editar aluno"
      ...
      <ConfirmDialog
        ...
        description={`${studentToRemove?.name ?? "Este aluno"} será removido de "${classDetail?.name ?? "esta turma"}". Essa ação não pode ser desfeita.`}
      ...
      <ConfirmDialog
        ...
        title="Excluir turma?"
        description={`"${classDetail?.name ?? "Esta turma"}" será excluída. Essa ação não pode ser desfeita.`}
        confirmLabel="Excluir"
        confirmingLabel="Excluindo..."
        isConfirming={deleteClass.isPending}
        errorMessage={deleteClass.isError ? getApiErrorMessage(deleteClass.error) : null}
