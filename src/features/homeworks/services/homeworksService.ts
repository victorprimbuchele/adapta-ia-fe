...
export const homeworksService = {
  ...
  // `<a href>` não envia o header Authorization, então o binário é buscado
  // via Axios e convertido em Blob (docs/API.md §6.3-PDF). `homeworkId` pode
  // ser a variante (não precisa de query) ou a geradora (exige learningProfileId).
  getHomeworkVariantPdf: (homeworkId: string): Promise<Blob> =>
    apiClient
      .get<Blob>(`/homeworks/${homeworkId}/pdf`, { responseType: "blob" })
      .then((r) => r.data),
};
