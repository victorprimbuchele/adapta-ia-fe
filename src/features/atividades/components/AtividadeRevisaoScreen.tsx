...
import { AlertCircle, Loader2, Mail, Send, Sparkles, Volume2 } from "lucide-react";
    ...
    <div className={`${cardCls} overflow-hidden flex flex-col`}>
      <div className="bg-brand px-4 py-3 flex items-center gap-2">
        ...
        <span className="text-xs text-white/70 flex-shrink-0">
          {destinatarios.length} aluno{destinatarios.length !== 1 ? "s" : ""}
        </span>
      </div>
...
export function AtividadeRevisaoScreen() {
  ...
  const { alunos } = useTurmaDetalhe(homework?.classId);
  ...
  const totalDestinatarios = alunos?.filter((a) => a.learningProfile).length ?? 0;
            ...
            {homework.adaptations.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                {homework.adaptations.map((variant) => {
                  const profile = perfis?.find((p) => p.id === variant.learningProfileId);
                  const prompt = profile ? parseProfilePrompt(profile.prompt) : null;
                  const destinatarios =
                    alunos?.filter((a) => a.learningProfile?.id === variant.learningProfileId) ?? [];
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-sm text-ink">
                    {isFullyReady ? "Tudo pronto para enviar" : "Alguns perfis ainda não estão prontos"}
                  </p>
                  <p className="text-xs text-muted mt-0.5">
                    {totalDestinatarios} aluno{totalDestinatarios !== 1 ? "s" : ""} receberá
                    {totalDestinatarios !== 1 ? "ão" : ""} a versão adaptada por e-mail
                  </p>
                </div>
              </div>
              <button
