...
import type { DeliveryRecipient, DeliveryRecipientStatus } from "../../../types/delivery";
...
function RecipientStatusPill({ status }: { status: DeliveryRecipientStatus }) {
  const config: Record<DeliveryRecipientStatus, { label: string; cls: string; Icon: typeof Loader2 }> = {
    pendente: { label: "Enviando...", cls: "bg-blue-50 text-blue-700", Icon: Loader2 },
    enviado: { label: "Enviado", cls: "bg-green-50 text-green-700", Icon: CheckCircle2 },
    falhou: { label: "Falhou", cls: "bg-red-50 text-red-700", Icon: AlertCircle },
  };
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${cls}`}>
      <Icon className={`w-3.5 h-3.5 ${status === "pendente" ? "animate-spin" : ""}`} />
      {label}
    </span>
  );
}
  ...
  return (
    <div className={`${cardCls} p-4 flex items-center justify-between gap-4`}>
      <div className="min-w-0">
        <p className="font-bold text-sm text-ink truncate">{recipient.studentName}</p>
...
export function AtividadeEnvioScreen() {
  const { id: homeworkId, deliveryId } = useParams<{ id: string; deliveryId: string }>();
  ...
  return (
    <AppLayout>
      <div className="p-8 max-w-3xl mx-auto">
        <BackBtn onClick={() => navigate(`/atividades/${homeworkId}/revisao`)} label="Revisão" />
          ...
          <>
            <div className="mb-8">
              <h1 className="text-2xl font-bold text-ink font-heading">
                {stillSending ? "Enviando atividade..." : failedCount > 0 ? "Envio com falhas" : "Envio concluído"}
              </h1>
              <p className="text-muted text-sm mt-1">
                {sentCount} de {recipients.length} aluno{recipients.length !== 1 ? "s" : ""} receberam a
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-sm text-ink">
                      {failedCount} e-mail{failedCount !== 1 ? "s" : ""} não chegou{failedCount !== 1 ? "ram" : ""}
                    </p>
            ...
            {resendDelivery.isError ? (
              <p className="text-sm text-red-600 mt-3">{getApiErrorMessage(resendDelivery.error)}</p>
            ) : null}
