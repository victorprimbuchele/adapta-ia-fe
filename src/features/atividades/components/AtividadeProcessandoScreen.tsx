...
import type { AdaptationStatus, ProfileAdaptationStatus } from "../../../types/homework";
...
function StatusPill({ status }: { status: AdaptationStatus }) {
  const config: Record<AdaptationStatus, { label: string; cls: string; Icon: typeof Clock }> = {
    pendente: { label: "Pendente", cls: "bg-slate-100 text-slate-500", Icon: Clock },
    processando: { label: "Processando", cls: "bg-blue-50 text-blue-700", Icon: Loader2 },
    concluido: { label: "Concluído", cls: "bg-green-50 text-green-700", Icon: CheckCircle2 },
...
export function AtividadeProcessandoScreen() {
  ...
  const { data: adaptationStatus, isPending: isStatusPending } = useAdaptationStatus(homeworkId);
  ...
  const profileName = (learningProfileId: string) =>
    perfis?.find((p) => p.id === learningProfileId)?.name ?? "Perfil de aprendizagem";
  ...
  const triggerErrorMessage =
    adaptHomework.isError && (!adaptationStatus || adaptationStatus.adaptations.length === 0)
      ? (ERROR_MESSAGES[getApiErrorCode(adaptHomework.error) ?? ""] ?? getApiErrorMessage(adaptHomework.error))
      : null;

  const adaptations = adaptationStatus?.adaptations ?? [];
  const hasAnyCompleted = adaptations.some((a) => a.status === "concluido");
  const stillWorking = adaptations.some((a) => a.status === "pendente" || a.status === "processando");
  ...
  return (
    <AppLayout>
      <div className="p-8 max-w-3xl mx-auto">
        ...
        <h1 className="text-2xl font-bold text-ink mb-1 font-heading">{homework?.title ?? "Atividade"}</h1>
        <p className="text-muted text-sm mb-8">
          A IA está adaptando o conteúdo para cada perfil de aprendizagem da turma. Isso pode levar alguns instantes.
        </p>

        {triggerErrorMessage ? (
          <div className={`${cardCls} p-8 text-center`}>
            <AlertCircle className="w-8 h-8 text-red-500 mx-auto mb-3" />
            <p className="text-sm text-ink font-medium mb-4">{triggerErrorMessage}</p>
            <button
              type="button"
          ...
          <div className="space-y-3">
            {adaptations.map((profile) => (
              ...
              >
                <div className="min-w-0">
                  <p className="font-bold text-sm text-ink truncate">{profileName(profile.learningProfileId)}</p>
                  {profile.status === "erro" && profile.failedReason ? (
                    <p className="text-xs text-red-600 mt-1">{profile.failedReason}</p>
                  ) : null}
                </div>
            ...
            {!stillWorking && hasAnyCompleted && adaptationStatus?.status === "erro" ? (
              <button
                type="button"
